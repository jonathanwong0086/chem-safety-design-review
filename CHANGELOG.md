...
本次改版吸收一批外部专家对真实报送专篇的评审意见，把"程序性一票否决、法定名录穿透、辨识/分级口径、缺陷四要素、模板残留反扫、活公式复核"这些能显著提升诊断质量的做法沉淀进技能，同时对易误判处加了防过激护栏。

### Added

- **可执行的"活公式"计算复核工作簿生成器** `scripts/build_calc_workbook.py`：由 JSON（或 `--demo`）生成 `.xlsx`，含重大危险源 q/Q、泄压面积 `A=10·C·V^(2/3)`、物料平衡闭合三张复核表加一张跨表联动的自校核汇总。黄色格为可改输入、浅灰为公式格，改输入后 Excel/LibreOffice 自动重算判定列；文档值与计算值并列做偏差判定。示例见 `examples/calc-input-sample.json` 与 `examples/计算复核-示例.xlsx`（均为脱敏占位数据）。
- **程序性一票否决门**（模板11 前置）：安全条件审查意见书缺失、依托设施未验收即投用、审查权限层级错配、备案/许可范围不符等命中任一，直接强制"不具备上会条件/不予通过"，对应 45 号令第十三/十九条。
- **法定名录穿透与法定定性一致性**（standard-anchors.md 类别11）：六类名录核验表（剧毒/易制毒/易制爆/监控/高毒/特别管控）带"动态目录红线"（现行主名录版本＋最近一次增补件文号），以及产品类别/行业分类/火灾危险性/重大危险源等级/剧毒监控判定的一致性校核。
- **模板残留/串项目专项扫描**（新增第10条专项、模板5C）：第5步先建本项目白名单，第8步反扫套用他项目车间号、错误项目名等残留。
- **缺陷四要素书写铁律**："位置—事实—依据—建议"四要素齐全方可成条。
- **意见修订审核表（模板12）与复核无误/编制亮点（模板13）**：闭环留痕（责任单位/优先级/修订情况/复核意见）与正面留痕。

### Changed

- **卡6 重写为"重大危险源辨识与分级"**：严格区分辨识层 `S=Σ(qi/Qi)`（S≥1 构成）与分级层 `R=ΣRj`（`Rj=βj·(qj/Qj)`，α 于最后调整 R），明确禁止写成 `R=α·Σβ·q/Q` 这类杂糅式；应急罐必计入最大存在量；临界量档位与危化品归类须自洽；单元边界优先于防火堤。
- **红线(六)加三条防过激护栏**：判"漏/删"前先核辨识口径；提取为空不等于内容缺失（待实证·调原件）；多路径独立复现＝高置信。
- SKILL.md 工作流第 5/8/11 步、专有清单（9→10 条）、参考索引同步更新。
